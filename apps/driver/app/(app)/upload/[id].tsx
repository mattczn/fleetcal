...
export default function UploadScreen() {
  ...
  const { data: docs = [], refetch, isLoading } = useQuery({
    ...
    queryFn:  () => fetchDocuments(id!, driver!.orgId),
    enabled:  !!id && !!driver,
  });
  ...
  function handleDelete(doc: LoadDocument) {
    Alert.alert("Delete document?", `Remove ${doc.fileName}?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          if (!driver) return;
          await deleteDocument(doc, driver.orgId, driver.name);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#1a73e8" }} edges={["top"]}>
      {/* Nav bar */}
      <View style={{ backgroundColor: "#1a73e8", flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 14, gap: 12 }}>
        <TouchableOpacity onPress={() => router.replace({ pathname: "/load/[id]", params: { id: id! } })} activeOpacity={0.7}
        ...
        <View style={{ flex: 1 }}>
          ...
          {load ? (
            <Text style={[txt(500), { fontSize: 12, color: "rgba(255,255,255,0.55)" }]}>
              {load.loadNum ? `Load #${load.loadNum}` : load.title}
            </Text>
          ) : null}
        </View>
      </View>
