/**
 * Railway API client for the dispatch (Expo / React Native) app.
 *
 * Mirrors apps/web/lib/railway.ts. Singleton pattern: one `railway` instance,
 * token retrieval wired via `setRailwayTokenProvider()` from a component
 * that has access to Clerk's `useAuth()` (see RailwayClientProvider).
 *
 * Base URL comes from `app.json` `extra.railwayApiUrl` (env.ts), defaulting
 * to the production URL. Set it to `http://<lan-ip>:8080` to point at a
 * locally-running `apps/api` from a physical device.
 */
...
const BASE_URL = env.railwayApiUrl ?? "https://fleetcalapi-production.up.railway.app";
...
class RailwayClient {
  private async req<T>(method: string, path: string, body?: unknown): Promise<T> {
    ...
    if (!res.ok) {
      // Read body once — calling .json() consumes the stream even on parse
      // failure, so we can't fall through to .text() afterwards.
  ...
  health() {
    return this.req<{ ok: true; service: string; version: string; timestamp: string }>("GET", "/v1/health");
  }
}
