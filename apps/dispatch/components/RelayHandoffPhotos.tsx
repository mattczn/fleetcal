/**
 * Relay handoff photos — dispatch (ops mobile) view.
 *
 * Drivers attach photos to relay loads (kind='relay_handoff'); ops sees
 * the same set here and can delete/upload. Mirrors the driver-app
 * component visually so the two surfaces feel consistent.
 */
...
export function RelayHandoffPhotos({ loadId }: { loadId: string }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      console.warn("[dispatch relay photos] list:", err);
    } finally {
      setLoading(false);
    }
  }, [loadId]);
    ...
    <View style={{ marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: "#ddd6fe" }}>
      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
        ...
        <TouchableOpacity onPress={addPhoto} disabled={busy}
          ...
          style={{
            ...
            borderRadius: 8, backgroundColor: "#6b21a8",
            opacity: busy ? 0.6 : 1,
          }}>
          {busy
      ...
      {loading ? (
        ...
        <TouchableOpacity onPress={addPhoto}
          ...
          <Camera size={14} color="#6b21a8" strokeWidth={2.2} />
          <Text style={[txt(700), { fontSize: 12, color: "#6b21a8" }]}>
            Drivers can attach photos here · none yet
          </Text>
        </TouchableOpacity>
      ) : (
            ...
            <TouchableOpacity key={p.id}
              ...
              {p.signedUrl
                ...
                : <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                    <Text style={[txt(700), { fontSize: 9, color: "#6b21a8" }]}>No URL</Text>
                  </View>}
            </TouchableOpacity>
          ))}
        </View>
