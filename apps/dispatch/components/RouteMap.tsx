...
<style>
  ...
  .pin {
    ...
    color: #fff; font-size: 11px; font-weight: 800; font-family: -apple-system,BlinkMacSystemFont,sans-serif;
    cursor: pointer;
  }
...
<script>
  ...
  window.initMap = function() {
    ...
    // Stop markers using AdvancedMarkerElement-style HTML overlays via OverlayView
    ...
    stops.forEach((s, i) => {
      ...
      // Driver check-in marker
      if (s.arrivedLat != null && s.arrivedLng != null) {
        const cinHtml = '<div class="checkin"></div>';
        const c = makeOverlay({ lat: s.arrivedLat, lng: s.arrivedLng }, cinHtml, null);
        checkinMarkersById[s.id] = c;
      }
    });
    ...
    <>
      ...
      >
        <View style={{ flex: 1, backgroundColor: "#1a73e8" }}>
          ...
          {/* Close — sits directly on the modal root, never gets pushed offscreen */}
          <TouchableOpacity
          ...
          {/* Stop counter pill — anchored to the bottom area above the card */}
          <View
            pointerEvents="none"
            style={{
              position: "absolute",
              bottom: insets.bottom + 12 + 92, // sits just above the card
              left: 0, right: 0,
              alignItems: "center",
            }}
          >
          ...
          {/* Bottom stop card — fixed compact height, sits at the bottom */}
          {selectedStop ? (
            <View
              style={{
                position: "absolute",
              ...
              <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {selectedStop.instructions ? (
                    <Text
                      style={[txt(500), { fontSize: 12, color: "#9a3412", marginTop: 4 }]}
                      numberOfLines={1}
                    >
                      {selectedStop.instructions}
                    </Text>
