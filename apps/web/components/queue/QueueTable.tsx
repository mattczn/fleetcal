...
export function QueueTable<R>({
  ...
  const visibleColumns = useMemo(() => {
    ...
    // Append any column the parent didn't include in order (newly added
    // columns) so we never silently drop them.
  ...
  // Columns panel state — slide-in from right edge.
  const [colsPanelOpen, setColsPanelOpen] = useState(false);

  return (
    <div className="flex h-full w-full min-h-0 min-w-0">
      <div className="flex-1 flex flex-col min-h-0 min-w-0 rounded-lg shadow-sm overflow-hidden"
        ...
        <div className="flex-1 overflow-auto min-h-0 min-w-0 relative" style={{ background: 'var(--gc-surface)' }}>
          <table style={{
            ...
            <thead>
              ...
              <tr>
                {selectable ? (
                  <th style={stHeaderCell} className="sticky top-0 z-20">
                    <input
                      type="checkbox"
              ...
              {visibleColumns.some(c => c.filter) || selectable ? (
                <tr>
                  {selectable ? <th style={stFilterCell} className="sticky z-10" /> : null}
                  {visibleColumns.map(c => (
                    <th key={c.key}
                      style={{ ...stFilterCell, position: 'sticky', top: 36, zIndex: 19, background: 'var(--gc-surface)' }}>
                      {c.filter ? (
                        <FilterInput
                          column={c}
                          value={filters[c.key] ?? ''}
                ...
                rows.map(r => {
                  ...
                  return (
                    <tr key={id}
                      ...
                      {selectable ? (
                        <td style={stBodyCell} onClick={e => e.stopPropagation()}>
                          <input type="checkbox"
      ...
      {/* Right-edge Columns tab */}
      {onHiddenColumnsChange ? (
        <>
          <button type="button"
            onClick={() => setColsPanelOpen(v => !v)}
            className="ml-0 flex items-center justify-center transition-colors"
            style={{
              width: 22,
              writingMode: 'vertical-rl',
              borderTopRightRadius: 6, borderBottomRightRadius: 6,
              background: colsPanelOpen ? 'var(--gc-hover)' : 'var(--gc-surface)',
              border: '1px solid var(--gc-border)',
              borderLeft: 'none',
              cursor: 'pointer',
            }}>
            <span className="text-[10px] font-semibold uppercase tracking-wider py-2"
              style={{ color: 'var(--gc-text-2)' }}>
              Columns
            </span>
          </button>
...
function ColumnsPanel<R>({
  columns, hiddenColumns, onHiddenColumnsChange,
  columnOrder, onColumnOrderChange, onClose,
}: {
  columns: QueueColumn<R>[];
  hiddenColumns: Set<string>;
  onHiddenColumnsChange: (next: Set<string>) => void;
  ...
  function reorder(from: string, to: string) {
    ...
    // Make sure every column has an entry; append missing ones.
  ...
  return (
    <div className="flex flex-col shrink-0 ml-0 rounded-r-lg shadow-sm"
      style={{
        width: 220,
        background: 'var(--gc-surface)',
        border: '1px solid var(--gc-border)',
      ...
      <div className="flex items-center justify-between px-3 py-2"
        ...
        <button type="button" onClick={onClose}
          style={{ color: 'var(--gc-text-3)', cursor: 'pointer' }}>
          <X size={14} />
        </button>
      </div>
