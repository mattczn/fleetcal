...
import { Calendar, Smartphone, FileCheck, CreditCard, Wallet, BarChart3, ArrowRight, ArrowLeft } from 'lucide-react';
...
const FLOW: ReadonlyArray<Step> = [
  ...
  { key: 'paperwork',  label: 'Paperwork verification', short: 'Paperwork',  href: '/product/paperwork',  Icon: FileCheck,  color: '#1e8e3e', light: '#e6f4ea', blurb: 'Verify the POD against the rate con, then release clean loads.' },
  { key: 'billing',    label: 'Billing',                short: 'Billing',    href: '/product/billing',    Icon: CreditCard, color: '#1a73e8', light: '#e8f0fe', blurb: 'Batch a week’s invoices and send them in a single sweep.' },
  { key: 'payroll',    label: 'Payroll',                short: 'Payroll',    href: '/product/payroll',    Icon: Wallet,     color: '#1e8e3e', light: '#e6f4ea', blurb: 'Driver events roll into weekly payroll, ready to finalize.' },
  { key: 'dashboard',  label: 'Insights',               short: 'Insights',   href: '/product/dashboard',  Icon: BarChart3,  color: '#1a73e8', light: '#e8f0fe', blurb: 'Revenue by truck, customer, and lane at a glance.' },
];
    ...
    <section style={{ background: '#f8f9fa', borderTop: '1px solid #e8eaed', padding: '64px 0' }}>
      <div className={WRAP}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <span className="font-mono" style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', color: '#1967d2' }}>Keep exploring</span>
            ...
            <Fragment key={s.key}>
              <Link
                ...
                style={{
                  ...
                  color: s.key === current ? '#202124' : '#5f6368',
                  background: s.key === current ? '#fff' : 'transparent',
                  border: `1px solid ${s.key === current ? '#dadce0' : 'transparent'}`,
                  borderRadius: 999,
                  padding: '7px 15px',
                  textDecoration: 'none',
                  whiteSpace: 'nowrap',
        ...
        <div className={`grid grid-cols-1 gap-5 ${prev && next ? 'md:grid-cols-2' : ''}`}>
          ...
          {next && <FlowCard step={next} dir="next" />}
        </div>
      </div>
    </section>
  );
...
function FlowCard({ step, dir }: { step: Step; dir: 'prev' | 'next' }) {
  ...
  return (
    <Link
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div className="font-mono" style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: step.color }}>{dir === 'prev' ? 'Previous step' : 'Next step'}</div>
      ...
      <span className="font-display hidden sm:inline-flex" style={{ alignItems: 'center', gap: 7, fontWeight: 600, fontSize: 15, color: step.color, whiteSpace: 'nowrap', flex: 'none' }}>
        {dir === 'prev' ? <><ArrowLeft size={16} strokeWidth={2.4} /> Back</> : <>Explore <ArrowRight size={16} strokeWidth={2.4} /></>}
      </span>
    </Link>
  );
}
