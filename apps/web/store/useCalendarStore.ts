...
export const useCalendarStore = create<CalendarStore>()(
  ...
  configureLegs: async (loadId, body) => {
    ...
    get().markLoadSelfWrite(loadId);
    try {
      // The reconcile is convergent and idempotent server-side: entries
      // without an eventId adopt any still-unclaimed leg in order and
      // only create when none is left, so re-sending the same payload
      // lands on the same rows. No client-side concurrency stamp is
      // needed — a stale cache heals by saving again.
